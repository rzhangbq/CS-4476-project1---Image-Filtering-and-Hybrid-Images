...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        kernel = kernel.reshape((1, kernel.shape[0], kernel.shape[0]))
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        filtered_image = torch.nn.functional.conv2d(input = x, weight = kernel, padding = pad, groups = self.n_channels)
    ...
    def forward(
        ...
        kernel = self.get_kernel(int(cutoff_frequency[0]))
        image1_low = self.low_pass(image1, kernel)
        image2_low = self.low_pass(image2, kernel)
