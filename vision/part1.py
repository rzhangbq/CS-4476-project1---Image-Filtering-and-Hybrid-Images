...
def pdf_of_standard_normal(x):
    return (1/(2*np.pi))*np.exp(-x**2/2)


def pdf_of_0centered_normal(x, sigma):
    return pdf_of_standard_normal(x/sigma)
...
def create_Gaussian_kernel_1D(ksize: int, sigma: int) -> np.ndarray:
    ...
    # print(ksize)
    kernel = np.zeros((ksize, 1), float)
    centre = ksize // 2
    for i in range(ksize):
        dis_from_centre = abs(i-centre)
        kernel[i] = pdf_of_0centered_normal(dis_from_centre, sigma)
...
def create_Gaussian_kernel_2D(cutoff_frequency: int) -> np.ndarray:
    ...
    ############################
    ### TODO: YOUR CODE HERE ###
    shape = cutoff_frequency * 4 + 1
    kernel_1d = create_Gaussian_kernel_1D(shape, cutoff_frequency)
    kernel = kernel_1d * np.reshape(kernel_1d, (1, shape))
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    zero_pad_0 = filter.shape[0]//2
    zero_pad_1 = filter.shape[1]//2
    zero_padded_image = np.pad(
        image, ((zero_pad_0, zero_pad_0), (zero_pad_1, zero_pad_1), (0, 0)))
    ...
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            for c in range(image.shape[2]):
                filtered_image[i][j][c] = np.sum(
                    zero_padded_image[i:i + filter.shape[0],j:j + filter.shape[1],c]*filter)
...
def create_hybrid_image(
    ...
    ############################
    ### TODO: YOUR CODE HERE ###
    image1_low = my_conv2d_numpy(image1, filter)
    ...
    return image1_low, image2_high, hybrid_image
